          ...
          <div className="fixed left-0 top-0 bottom-0 w-[85%] max-w-[340px] bg-white shadow-2xl z-[60] overflow-hidden flex flex-col animate-slide-in-left">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 bg-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[14px] font-bold text-black tracking-tight leading-none">Shop by Categories</h2>
                  <p className="text-[10px] text-gray-400 font-medium tracking-widest uppercase mt-0.5">Explore collections</p>
                </div>
              </div>
              <button
                onClick={() => setShowCategoriesDrawer(false)}
                className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-200 hover:bg-gray-50 active:scale-90 transition-all"
              >
            ...
            <div className="flex-1 overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden bg-white">
              {navigationCategories.length > 0 ? (
                    ...
                    return (
                      <div key={category.id}>
                        ...
                        <button
                          ...
                          className="flex items-center justify-between w-full px-5 py-4 text-left bg-white active:bg-gray-50 transition-colors"
                        >
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 border transition-all duration-200 ${
                              isExpanded ? "bg-black border-black" : "bg-white border-gray-200"
                            }`}>
                              <CategoryIcon className={`w-4 h-4 ${isExpanded ? "text-white" : "text-gray-500"}`} />
                            </div>
                            <div className="flex flex-col items-start">
                              <span className={`text-[14px] font-semibold leading-tight ${isExpanded ? "text-black" : "text-gray-800"}`}>
                                {category.name}
                              </span>
                          ...
                          {category.subcategories.length > 0 && (
                            <ChevronDown className={`w-4 h-4 flex-shrink-0 transition-transform duration-200 ${isExpanded ? "rotate-180 text-black" : "text-gray-400"}`} />
                          )}
                        </button>
                                  ...
                                  {hasChildren ? (
                                    <>
                                      <button
                                        ...
                                        className="flex items-center justify-between w-full px-6 py-3.5 text-left bg-transparent active:bg-gray-100 transition-colors"
                                      >
                                        <div className="flex items-center gap-3">
                                          <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 transition-colors ${isSubExpanded ? "bg-black" : "bg-gray-300"}`} />
                                          <span className={`text-[13px] font-semibold ${isSubExpanded ? "text-black" : "text-gray-600"}`}>
                                            {subcategory.name}
                                          </span>
                                        </div>
                                        <ChevronDown className={`w-3.5 h-3.5 flex-shrink-0 transition-transform duration-200 ${isSubExpanded ? "rotate-180 text-black" : "text-gray-400"}`} />
                                      </button>
                                      ...
                                      {isSubExpanded && (
                                        <div className="bg-white border-t border-gray-100 divide-y divide-gray-50">
                                          <Link
                                            ...
                                            className="flex items-center gap-3 px-9 py-3 text-[12px] font-bold text-black hover:bg-gray-50 transition-colors"
                                            onClick={() => setShowCategoriesDrawer(false)}
                                          >
                                          ...
                                          {subcategory.childCategories.map((childCategory) => (
                                            <Link
                                              ...
                                              className="flex items-center gap-3 px-9 py-3 text-[12px] font-medium text-gray-500 hover:text-black hover:bg-gray-50 transition-colors"
                                              onClick={() => setShowCategoriesDrawer(false)}
                                            >
                                    ...
                                    <Link
                                      to={`/all-products?subcategoryId=${subcategory.id}`}
                                      className="flex items-center gap-3 px-6 py-3.5 text-[13px] font-medium text-gray-600 hover:text-black hover:bg-white transition-colors"
                                      onClick={() => setShowCategoriesDrawer(false)}
                                    >
                ...
                <div className="flex flex-col items-center justify-center py-20 px-10 text-center">
                  <div className="w-16 h-16 border-2 border-gray-100 rounded-2xl flex items-center justify-center mb-5">
                    <Grid className="w-7 h-7 text-gray-300" />
                  </div>
                  <h3 className="text-base font-bold text-gray-900 mb-1">No Categories Found</h3>
                  <p className="text-xs text-gray-400">Couldn't load categories. Please try again.</p>
                </div>
              )}
            </div>
            ...
            <div className="px-5 py-4 border-t border-gray-200 bg-white">
              <Link
                to="/all-products"
                className="flex items-center justify-center gap-2.5 w-full py-3.5 bg-black text-white rounded-xl text-[13px] font-bold tracking-wide active:scale-95 transition-all"
                onClick={() => setShowCategoriesDrawer(false)}
              >
