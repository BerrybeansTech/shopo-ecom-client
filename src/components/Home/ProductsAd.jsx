  ...
  useEffect(() => {
    const fetchPromoBanner = async () => {
      try {
        ...
        // Place 2 matches position === 'place_2' (or second active banner)
        const place2Banner = activeBanners.find((b) => b.position === "place_2") || activeBanners[1];
        if (place2Banner) {
          setPromoBanner(place2Banner);
        } else {
          setPromoBanner(null);
          setErrorMsg("No active promotional banner uploaded");
        }
      } catch (err) {
        console.error("Error fetching promo banner (Place 2):", err);
        setPromoBanner(null);
