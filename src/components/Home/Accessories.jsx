...
export default function Accessories({ className = "" }) {
  ...
  const mapLocalProductToApiProduct = (localProduct) => {
    ...
    return {
      ...
      category: { name: localProduct.category || localProduct.mainCategory || "Accessories" },
  ...
  useEffect(() => {
    const fetchAccessories = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await productApi.getAccessories();

        const productsData = response?.data || response || [];
        if (Array.isArray(productsData) && productsData.length > 0) {
          setProducts(productsData.slice(0, 12));
        } else {
          setError("No accessories products available.");
        }
      } catch (error) {
        console.error("Error fetching Accessories products:", error);
        setError("Failed to load accessories products. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className={`bg-[#f8f8f899] py-16 ${className}`}>
        <div className="container-x mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold text-black mb-6">Accessories</h2>
